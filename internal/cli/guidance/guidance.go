...
const ScriptContract = `## Script contract
...
` + "`workflow(nameOrRef, args)`" + ` composes one child workflow by existing path or
...
const WorkflowShape = `## Workflow shape
...
const QualityPatterns = `## Quality patterns
